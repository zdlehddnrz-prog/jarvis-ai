...
항상 ${lnames[lang] || '한국어'}로 답변하고 출처를 명시하세요.`
    ...
    while (iterations < 8) {
      ...
      const res = await openai.chat.completions.create({
        model: 'gpt-4o', max_tokens: 2048, tools, tool_choice: 'auto',
        messages: currentMessages,
      })
      const choice = res.choices[0]
      if (choice.finish_reason === 'stop') return Response.json({ reply: choice.message.content })
